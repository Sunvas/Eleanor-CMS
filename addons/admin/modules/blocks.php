...
/*
	—татусы блоков:
		-3 - блок ожидает наступлени€ даты начала показа
		-2 -  блок не отображаетс€, поскольку наступила дата завершени€ показов
		-1 - зарезервировано
		0 - блок заблокирован
		1 - блок активирован
*/
...
else
{
	...
	if($_SERVER['REQUEST_METHOD']=='POST' and Eleanor::$our_query)
	{
		...
		if(is_int($gid))
		{
			...
			#”дал€ем возможно удаленные темы
...
{global$Eleanor,$title;
	...
	$ids=array(
		'user'=>array('user'=>array('t'=>$lang['bydef'],'g'=>true)),#t - название идентификатора, g - признак наличи€ группы у идентификатора
