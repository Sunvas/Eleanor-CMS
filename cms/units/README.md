 ...
 * Файл вида `unit_name.php` который должен возвращать объект, который реализует интерфейсы по которым система определяет возможности юнита. Например, если юнит доступен из пользовательской части, они должен реализовать интерфейс `UserSpace`; если доступен из дашборда - должен реализовать интерфейс `Admin`; если должен запускаться в фоне - интерфейс `Cron` и т.п. Поскольку файлы юнитов могут включаются по несколько раз, рекомендуется делать их небольшими, вынося всю логику в одноимённый каталог.
