...
{	public function Process()
	{		if(!isset($_POST['session']))
		...
		#«амен€ем все [ и ] потому что они криво вставл€ютс€ в тег [attach]
		...
		if((!isset($this->previews) and !empty($_POST['dopreviews']) or $this->previews) and in_array(strtolower(pathinfo($path,PATHINFO_EXTENSION)),$this->preview))
			try
			{				Image::Preview(
					...
					array(
						...
						'first'=>$this->vars['thumb_first'],#„то будет уменьшатьс€ первое: высота или ширина. w,h . јвтоматически: b - по наибольше стороне, s - по наименьше стороне.
