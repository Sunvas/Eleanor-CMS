...
$near=$reads=array();
$R=Eleanor::$Db->Query('SELECT `p`.`id`,`p`.`reads` FROM `'.$Eleanor->module['config']['rt'].'` `rt` INNER JOIN `'.$Eleanor->module['config']['t'].'` `p` ON `rt`.`id`=`p`.`id` WHERE `rt`.`tag`'.Eleanor::$Db->In($Eleanor->module['ptags']));
while($a=$R->fetch_row())
...
$R=Eleanor::$Db->Query('SELECT `id`,`uri`,`lcats`,`title` FROM `'.$Eleanor->module['config']['tl'].'` WHERE `id`'.Eleanor::$Db->In($near).' AND `lstatus`=1');
if($R->num_rows>0)
{	echo'<ul>';
