...
{	protected static function Menu($act='')
	{		$lang=Eleanor::$Language['lc'];
		...
		$GLOBALS['Eleanor']->module['navigation']=array(
			array($links['list'],$lang['list'],'act'=>$act=='list',
				'submenu'=>$links['news']
					? array(
						array($links['news']['link'],$lang['news']($links['news']['cnt'])),
					)
					: false,
			),
	...
	*/
	public static function CommentsList($items,$modules,$titles,$cnt,$pp,$qs,$page,$links,$ong)
	{		if(!$ong)
		...
		$Lst=Eleanor::LoadListTemplate('table-list',$ong ? 6 : 7)
			->begin(
				array($lang['date'],70,'sort'=>$qs['sort']=='date' ? $qs['so'] : false,'href'=>$links['sort_date']),
				array($lang['author'],70,'sort'=>$qs['sort']=='author' ? $qs['so'] : false,'href'=>$links['sort_author']),
				array($lang['published'],'sort'=>$qs['sort']=='module' ? $qs['so'] : false,'href'=>$links['sort_module']),
...
<table class="tabstyle tabform" id="ftable">
	<tr class="infolabel"><td colspan="2"><a href="#">'.$lang['filter'].'</a></td></tr>
	<tr>
		<td><b>'.$lang['module'].'</b><br />'.Eleanor::Select('fi[module]',$omods).'</td>
...
$(function(){
	...
	*/
	public static function Edit($id,$module,$info,$values,$bypost,$error,$back,$links)
	{		static::Menu();
		...
		return Eleanor::$Template->Cover($Lst->form()
			->begin()
			->item($lang['module'],$module)
			->item($lang['published'],'<a href="'.$info[1].'" target="_blank">'.$info[0].'</a>')
			->item($lang['date'],Eleanor::$Language->Date($values['date'],'fdt'))
			->item($lang['author'],$values['author_id'] ? '<a href="'.Eleanor::$Login->UserLink(htmlspecialchars_decode($values['author'],ELENT),$values['author_id']).'">'.$values['author'].'</a>' : $values['author'])
			->item($lang['text'],$GLOBALS['Eleanor']->Editor->Area('text',$values['text'],array('bypost'=>$bypost)))
			->item($lang['status'],Eleanor::Select('status',Eleanor::Option($ltpl['activate'],1,$values['status']==1).Eleanor::Option($ltpl['deactivate'],0,$values['status']==0).Eleanor::Option($ltpl['waiting_act'],-1,$values['status']==-1)))
	...
	*/
	...
	{
		static::Menu();
		return Eleanor::$Template->Cover(Eleanor::$Template->Confirm(sprintf(Eleanor::$Language['lc']['deleting'],Strings::CutStr(strip_tags($a['text']),200)),$back));
	}
