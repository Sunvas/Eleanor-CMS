	...
	*/	protected static function Menu($act='')
	{		$lang=Eleanor::$Language['modules'];
		...
		$items массив модулей. Формат: id=>array(), ключи внутреннего массива:
			...
			protected - флаг защищенности модули (защищенные модули - это чаще всего системные модули)
			path - каталог модуля
	...
	*/
	...
	/*
		...
		$values массив значений полей:
			...
			multiservice - флаг мультисервисного модуля (мультисервисный модуль, это модуль, файлы для запуска которого находится в каталогах с именами сервисов внутри главного каталога модуля)
	...
	public static function AddEdit($id,$values,$errors,$back,$links)
	{		static::Menu($id ? '' : 'add');
		...
		if(Eleanor::$vars['multilang'])
		{
			...
			$ml=array(
				...
				'descr'=>Eleanor::Text('descr',$values['descr'],array('tabindex'=>3)),
			);
