...
if($m)
{
	...
	if(!isset($Eleanor->modules['ids'][$m]))
		return ExitPage(404);
	$R=leanor::$Db->Query('SELECT `id`,`services`,`sections`,`title_l`,`path`,`multiservice`,`file`,`files`,`image`,`user_groups` FROM `'.P.'modules` WHERE `id`='.(int)$Eleanor->modules['ids'][$m].' AND `active`=1 LIMIT 1');
	if(!$a=$R->fetch_assoc())
		return ExitPage(404);
	if($a['user_groups'])
	{
